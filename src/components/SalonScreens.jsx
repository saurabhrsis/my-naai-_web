...
function queueCardTone(item) {
  if (isSameDate(item.bookingDate, 0)) return 'today';
  if (isSameDate(item.bookingDate, 1)) return 'tomorrow';
  return 'later';
}
...
export function SalonQueueScreen({ session, navigate, notify }) {
  ...
  return <div className="screen salon-queue-screen"><PageHeader title="Customer queue" subtitle="Keep every chair moving smoothly." action={<div className="page-actions"><Button size="small" className="walkin-button" onClick={() => setWalkInOpen(true)}><UserPlus size={15} /> Walk-in</Button><button className="notification-button" onClick={() => navigate('notifications')} aria-label="Open notifications"><Bell size={18} /></button><button className="refresh-icon-button" onClick={() => load(true)} aria-label="Refresh queue"><Zap size={17} /></button></div>} /><div className="queue-stats"><StatCard icon={UsersRound} label="In queue" value={items.length} note="active appointments" tone="gold" /><StatCard icon={Clock3} label="Next up" value={items[0]?.queueNumber || '—'} note={items[0]?.bookingTime ? formatTime(items[0].bookingTime) : 'no wait'} tone="green" /><StatCard icon={CalendarCheck2} label="Today" value={items.filter(item => isSameDate(item.bookingDate, 0)).length} note="appointments" /></div>{loading ? <div className="list-stack">{[1, 2, 3, 4].map(item => <SkeletonCard key={item} className="queue-skeleton" />)}</div> : grouped.length ? <div className="queue-groups">{grouped.map(group => <section className="queue-group" key={group.label}><div className="queue-group-heading"><h2>{group.label}</h2><span>{group.items.length} {group.items.length === 1 ? 'booking' : 'bookings'}</span></div>{group.items.map(item => <article className="queue-card" key={item.bookingId}><div className={cx('queue-token', queueCardTone(item))}><small>TOKEN</small><strong>{item.queueNumber || '—'}</strong></div><div className="queue-main"><div className="queue-card-heading"><div><h3>{item.userName || 'Guest'}</h3><span>{item.bookingDate ? `${formatDate(item.bookingDate)} · ${formatTime(item.bookingTime)}` : 'Appointment time pending'}</span></div><Button size="small" onClick={() => markDone(item.bookingId)} loading={doneId === item.bookingId}>Done</Button></div><div className="queue-meta"><span><Scissors size={14} /> {item.serviceNames || item.services || 'Salon service'}</span>{item.barberName && <span><UserRound size={14} /> {item.barberName}</span>}{item.userPhone && item.userPhone !== '0000000000' && <a href={`tel:${item.userPhone}`}><Phone size={14} /> {item.userPhone}</a>}</div></div></article>)}</section>)}</div> : <EmptyState icon={UsersRound} title="No customers in queue" message="New booking requests will appear here." />}<WalkInModal open={walkInOpen} onClose={() => setWalkInOpen(false)} session={session} notify={notify} onAdded={item => setItems(current => [...current, item])} /></div>;
}
...
function normalizeService(item, index) { return { id: item.serviceId || item.id || `new-service-${index}`, serviceId: item.serviceId || item.id || `new-service-${index}`, name: item.serviceName || item.name || '', price: item.price || '', duration: String(item.durationMinutes || item.duration || 30), description: item.description || '' }; }
function normalizeBarber(item, index) { return { id: item.barberId || item.id || `new-barber-${index}`, barberId: item.barberId || item.id || `new-barber-${index}`, name: item.fullName || item.name || '', image: item.profileImageUrl || item.image || '', rating: item.ratingAverage || item.rating || '0.0', isAvailable: item.isAvailable ?? true }; }
...
export function BookingRequestScreen({ params, navigate, notify, session }) {
  ...
  const [delayOpen, setDelayOpen] = useState(Boolean(params?.openDelayModal));
  ...
  const action = async (value, delayMinutes) => { setActionLoading(value); try { if (!isDemo) { const response = await api.bookingRequestOwnerAction(params.bookingRequestId, value === 'DELAY' ? { action: value, delayMinutes: String(delayMinutes) } : { action: value }); if (response?.status && response.status !== 'SUCCESS') throw new Error(response.message || 'Could not update request'); } notify?.('success', value === 'ACCEPT' ? 'Booking accepted.' : value === 'REJECT' ? 'Booking rejected.' : `Booking delayed by ${delayMinutes} minutes.`); setDelayOpen(false); navigate('queue'); } catch (error) { notify?.('error', getErrorMessage(error, 'Could not update booking request.')); } finally { setActionLoading(''); } };
  ...
  return <div className="screen booking-request-screen"><PageHeader title="New booking request" subtitle="A customer is waiting for your response." onBack={() => navigate('queue')} /><div className="request-card"><div className="request-card-top"><div className="request-avatar">{getInitials(details?.customerName || 'Guest')}</div><div><span className="eyebrow">CUSTOMER</span><h2>{details?.customerName || 'Guest'}</h2><span className="request-status"><i /> Needs your response</span></div></div><div className="request-detail-grid"><div><CalendarDays size={17} /><span><small>Selected date</small><strong>{formatDate(details?.bookingDate)}</strong></span></div><div><Clock3 size={17} /><span><small>Time slot</small><strong>{details?.startTime || '—'} – {details?.endTime || '—'}</strong></span></div><div><Scissors size={17} /><span><small>Services</small><strong>{details?.services || 'Salon service'}</strong></span></div></div></div><div className="request-actions"><div><Button variant="success" loading={actionLoading === 'ACCEPT'} onClick={() => action('ACCEPT')}>Accept <Check size={17} /></Button><Button variant="danger" loading={actionLoading === 'REJECT'} onClick={() => action('REJECT')}>Reject <X size={17} /></Button></div><Button variant="warning" loading={actionLoading === 'DELAY'} onClick={() => setDelayOpen(true)}>Suggest a delay <Clock3 size={17} /></Button></div><Modal open={delayOpen} onClose={() => setDelayOpen(false)} title="Suggest a delay"><p className="modal-lede">Give the customer a little more time. They will be able to accept or decline the new slot.</p><div className="delay-options">{[20, 40, 60].map(minutes => <button key={minutes} onClick={() => action('DELAY', minutes)} disabled={Boolean(actionLoading)}><Clock3 size={17} /><span><strong>+{minutes} minutes</strong><small>Suggest a later appointment</small></span><ChevronRight size={16} /></button>)}</div></Modal></div>;
}

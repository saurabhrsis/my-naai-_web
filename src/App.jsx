...
function AuthFlow({ onComplete, notifyInstall }) {
  ...
  if (view === 'register') return <SalonRegistration onBack={() => setView('login')} onComplete={onComplete} notifyInstall={notifyInstall} demo={() => demo('SALON')} />;
...
function SalonRegistration({ onBack, onComplete, notifyInstall, demo }) {
  const [step, setStep] = useState('phone');
  const [mobile, setMobile] = useState('');
  const [otp, setOtp] = useState('');

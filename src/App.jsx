...
function AuthFlow({ onComplete, notifyInstall }) {
  ...
  const verify = async event => {
    ...
    try {
      ...
      onComplete({ role, token: user.token, user, userId: user.userId || user.salon?.salonId || user.salonId, isNewSalon });
  ...
  const createAccount = async event => {
    ...
    setBusy(true); setError('');
    try { const deviceToken = pushToken || await requirePushToken(); const response = await api.userOnBoard(withDeviceToken({ phoneNumber: mobile, fullName: name.trim() }, deviceToken)); if (response?.status !== 'SUCCESS') throw new Error(response?.message || 'Could not create account.'); onComplete({ role: 'USER', token: response.data?.token, user: response.data, userId: response.data?.userId }); } catch (createError) { setError(getErrorMessage(createError, 'Could not create your account.')); } finally { setBusy(false); }
  };
...
function SalonRegistration({ onBack, onComplete, notifyInstall }) {
  ...
  const request = async event => { event.preventDefault(); if (!/^\d{10}$/.test(mobile)) return setError('Enter a valid 10-digit mobile number.'); setBusy(true); setError(''); try { const token = await requirePushToken(); setPushToken(token); const response = await api.salonOwnerLogin({ phoneNumber: mobile }); if (response?.status !== 'SUCCESS') throw new Error(response?.message || 'Could not send OTP.'); setStep('otp'); setOtp(''); setResendSeconds(30); } catch (requestError) { setError(getErrorMessage(requestError, 'Could not send OTP.')); } finally { setBusy(false); } };

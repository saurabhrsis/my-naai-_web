...
- Time-critical alerts (a salon booking request, a delay proposal) sound a repeating **booking buzzer** the way the mobile app does, and pulse the device. The buzzer uses the Web Audio API (quadratic/square tone) and is unlocked on the first user gesture because browsers block audio before an interaction; the notification itself also carries a `vibrate` pattern so it shakes on support Android browsers even in the background. When the PWA is installed and closed, `firebase-messaging-sw.js` still delivers background alerts and now adds the same buzzer vibration — the audible sound in the background is whatever notification sound the server's push payload requests, since a closed service worker cannot synthesize a custom tone (`src/lib/buzzer.js`).
